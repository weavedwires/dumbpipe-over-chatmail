...
Секретный ключ можно переиспользовать между запусками: если переменная окружения ```IROH_SECRET``` не задана, dumbpipe ищет ключ в файле ```iroh-secret.txt``` в текущей директории, а если файла нет — генерирует случайный. Переменная окружения имеет приоритет над файлом. Чтобы один раз сгенерировать ключ, сохранить его в ```iroh-secret.txt``` и получить тикет:

```
dumbpipe save-ticket
```

После этого любой запуск ```dumbpipe listen``` / ```connect``` из той же директории будет использовать тот же ключ, а ранее выданный тикет останется действительным. В оригинале такого нет — там используется только переменная окружения либо случайный одноразовый ключ.
